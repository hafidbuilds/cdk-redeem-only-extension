(function attachMultiPageVerificationResendController(root, factory) {
  ...
  function createVerificationResendController(context = {}) {
    ...
    const isCustomEmailNonVerificationNotificationError = (...args) => context.isCustomEmailNonVerificationNotificationError(...args);
    ...
    const normalizeDigits = (...args) => context.normalizeDigits(...args);
    ...
    function validateFetchedVerificationCode(result = {}, step = 0) {
      if (result?.codeStageComplete || result?.alreadyOnNewPasswordPage || result?.emailAlreadyVerified) return result;
      const code = normalizeDigits(result?.code);
      if (!code) throw new Error(`步骤 ${step}：取到的验证码不是有效六位数字，已忽略占位验证码。`);
      return { ...result, code };
    }
        ...
        async function resolveCustomEmailVerificationStep(step, state = {}, options = {}) {
          ...
          let delayedMailFirstAttempt = 0;
          let delayedMailResendRequested = false;
          ...
          for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
            ...
            try {
              ...
              const nonVerificationNotificationError = step === 4
                && isCustomEmailNonVerificationNotificationError(err);
              const delayedMailError = assurivoEmptyFeedError || nonVerificationNotificationError;
              if (delayedMailError && !delayedMailFirstAttempt) {
                delayedMailFirstAttempt = attempt;
                ...
                const firstWaitMessage = assurivoEmptyFeedError
                  ? `Assurivo JSON 接口当前未收到本轮验证码邮件(data=[]/无新邮件/无 ChatGPT 邮件)，将最多继续等待 ${Math.round(STEP4_ASSURIVO_EMPTY_FEED_WAIT_MS / 1000)} 秒。`
                  : '自定义邮箱最新邮件是 OpenAI 登录通知，不是验证码邮件；先继续等待验证码邮件到达。';
                await addLog(`步骤 ${completionStep}：${firstWaitMessage}`, 'warn');
              }
              if (delayedMailError && !delayedMailResendRequested && attempt >= Math.max(2, delayedMailFirstAttempt + 1)) {
                delayedMailResendRequested = true;
                try {
                  ...
                  const resendReason = assurivoEmptyFeedError ? 'Assurivo 连续未返回邮件' : '连续读到 OpenAI 登录通知';
                  await addLog(`步骤 ${completionStep}：${resendReason}，已请求重新发送一封新的${getVerificationCodeLabel(step)}验证码，将继续等待。`, 'warn');
                } catch (resendError) {
                  if (isStopError(resendError)) throw resendError;
                  const resendFailureReason = assurivoEmptyFeedError ? 'Assurivo 暂无邮件' : '最新邮件仍是登录通知';
                  await addLog(`步骤 ${completionStep}：${resendFailureReason}，尝试重新发送验证码失败，将继续等待邮箱新邮件：${resendError.message}`, 'warn');
                }
              }
        ...
        async function fetchVerificationCodeOnly(step, state = {}, mail = null, options = {}) {
          ...
          if (customResult?.handled) {
            return {
              ...validateFetchedVerificationCode(customResult, completionStep),
              handled: true,
              stateKey,
            };
          }
          ...
          return {
            ...validateFetchedVerificationCode(result || {}, completionStep),
            handled: true,
            stateKey,
          };
        }
