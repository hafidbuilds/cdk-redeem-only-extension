(function attachMultiPageAssurivoFeedClient(root, factory) {
  ...
  function createAssurivoFeedClient(context = {}) {
    ...
    const getCompletionStep = (...args) => context.getCompletionStep(...args);
        ...
        function isRetryableCustomEmailVerificationFetchError(error) {
          ...
          return error?.code === 'CUSTOM_EMAIL_LATEST_NON_VERIFICATION'
            || /HTTP\s*(?:408|429|5\d\d)\b|Gateway Time-out|gateway timeout|暂未返回有效验证码|自定义邮箱暂未返回有效验证码|取码接口.*(?:超时|timeout)|查询暂时较慢/i.test(message);
        }
        ...
        function getCustomEmailPayloadSubject(payload) {
          if (payload && typeof payload === 'object') {
            const directSubject = payload.subject || payload.title || payload.mail_subject || payload.email_subject;
            if (directSubject) {
              return String(directSubject).replace(/\s+/g, ' ').trim();
            }
          }
          if (typeof payload !== 'string') {
            return '';
          }
          const titleMatch = payload.match(/<title\b[^>]*>([\s\S]*?)<\/title>/i);
          if (!titleMatch) {
            return '';
          }
          return titleMatch[1]
        ...
        function classifyCustomEmailNonVerificationNotification(payload) {
          const subject = getCustomEmailPayloadSubject(payload);
          if (/^new\s+sign[- ]in\s+to\s+your\s+(?:openai|chatgpt)\s+account\b/i.test(subject)) {
            return {
              kind: 'openai-sign-in-notification',
              subject,
            };
          }
          return null;
        ...
        async function fetchCustomEmailVerificationCode(step, state = {}, options = {}) {
          ...
          for (let requestIndex = 0; requestIndex < requests.length; requestIndex += 1) {
            ...
            const nonVerificationNotification = classifyCustomEmailNonVerificationNotification(payload);
            if (nonVerificationNotification) {
              lastError = new Error(
                `步骤 ${completionStep}：${request.label}最新邮件为 OpenAI 登录通知“${nonVerificationNotification.subject}”，不是验证码邮件，已忽略并等待新的验证码邮件。`
              );
              lastError.code = 'CUSTOM_EMAIL_LATEST_NON_VERIFICATION';
              lastError.mailSubject = nonVerificationNotification.subject;
              if (requestIndex < requests.length - 1) {
                await addLog(`${request.label}最新邮件是登录通知，将回退到下一种取码方式。`, 'warn');
                continue;
              }
              throw lastError;
            }
    ...
    return {
      ...
      getCustomEmailPayloadSubject,
      classifyCustomEmailNonVerificationNotification,
      isCustomEmailNonVerificationNotificationError,
