(function attachBackgroundSetGptPassword(root, factory) {
  ...
  function createSetGptPasswordExecutor(deps = {}) {
    ...
    function isPasswordUpdatedPageState(snapshot = {}) {
      return normalizeString(snapshot?.state || snapshot?.pageState).toLowerCase() === 'password_updated_page'
        || snapshot?.gptPasswordSet === true
        || snapshot?.confirmedBySuccessMessage === true;
    }
    ...
    async function recoverSetPasswordAuthRetryPageFromBackground(tabId, visibleStep, snapshot = {}) {
      ...
      async function resolveRecoveredPageState(recoverResult = {}) {
        ...
        if (['new_password_page', 'new_password_loading', 'password_updated_page'].includes(recoveredState)) {
          return {
            state: recoveredState,
            url: normalizeString(recoverResult?.url || snapshot?.url),
            errorText: normalizeString(recoverResult?.errorText),
            successText: normalizeString(recoverResult?.successText),
          };
        }
      ...
      try {
        ...
        if (isPasswordUpdatedPageState(recoveredPageState)) {
          await addStepLog(visibleStep, '设置 GPT 密码：Try again 恢复消息异常，但页面已显示密码更新成功。', 'success');
      ...
      if (isPasswordUpdatedPageState(recoveredPageState)) {
        await addStepLog(visibleStep, '设置 GPT 密码：Try again 后页面已显示密码更新成功。', 'success');
        return createPasswordSubmitSuccess(recoveredPageState, {
          recoveredAuthRetryPage: true,
          confirmedBySuccessMessage: true,
        });
    ...
    async function waitForPasswordSubmitOutcomeFromBackground(tabId, visibleStep, expectedEmail = '') {
      ...
      while (Date.now() - startedAt < PASSWORD_SUBMIT_CONFIRM_TIMEOUT_MS) {
        ...
        if (isPasswordUpdatedPageState(pageState)) {
          await addStepLog(visibleStep, '设置 GPT 密码：后台轮询检测到页内密码更新成功提示。', 'success');
          return createPasswordSubmitSuccess(pageState, {
    ...
    async function fetchPasswordSetupCode(state = {}, mail = null, options = {}) {
      ...
      if (!/^\d{6}$/.test(code) || /^0{6}$/.test(code)) {
        throw new Error('设置 GPT 密码验证码无效，已拒绝提交占位验证码。');
      }
      return {
        ...result,
        code,
