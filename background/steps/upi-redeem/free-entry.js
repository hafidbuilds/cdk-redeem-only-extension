(function attachMultiPageUpiRedeemFreeEntry(root, factory) {
...
})(typeof self !== 'undefined' ? self : globalThis, function createMultiPageUpiRedeemFreeEntryModule(root) {
  function createUpiRedeemFreeEntry(context = {}) {
    ...
    const cleanupModule = context.freeEntryCleanupModule
      || root.MultiPageUpiRedeemFreeEntryCleanup
      || (typeof require === 'function' ? require('./free-entry-cleanup.js') : null);
    if (!cleanupModule?.createUpiRedeemFreeEntryCleanup) {
      throw new Error('UPI Redeem Free entry cleanup module is unavailable.');
    }
    const {
    ...
    } = cleanupModule.createUpiRedeemFreeEntryCleanup({
      normalizeString,
      parseCdkeyPoolText,
