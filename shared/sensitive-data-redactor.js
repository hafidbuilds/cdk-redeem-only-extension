(function attachSensitiveDataRedactor(root, factory) {
  ...
  function describeCdkey(value = '', channel = 'upi') {
    return `cdkey:${String(channel || 'upi').trim().toLowerCase()}:${stableFingerprint(value)}`;
  }

  return { SENSITIVE_KEY, describeCdkey, mask, redactSensitiveData, redactText, stableFingerprint };
});
