...
test('verification code extraction rejects all-zero placeholders but keeps real six-digit codes', () => {
  const placeholder = verificationCodeExtractor.extractStrictVerificationCodeFromBody(
    'Your ChatGPT verification code is 000000.'
  );
  const valid = verificationCodeExtractor.extractStrictVerificationCodeFromBody(
    'Your ChatGPT verification code is 123456.'
  );

  assert.deepEqual(placeholder.codes, []);
  assert.deepEqual(valid.codes, ['123456']);
});
