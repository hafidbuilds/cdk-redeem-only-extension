...
## Free Account Tool V3.0.0

V3.0.0 将扩展收敛为 Free 账号注册、凭据管理和试用资格检测工具，彻底移除 CDK 兑换和 Plus 分类。

### 版本信息

- Manifest 版本：`3.0.0`
- 版本名称：`Free Account Tool V3.0.0`
- 结果 schema：`freeAccountResults` / `schemaVersion: 3`
- 运行架构：Chrome Manifest V3 Service Worker + Side Panel

### Free 账号模型

- 注册工作流升级为 `workflowVersion: 3`：三条路线统一为九步，侧栏隐藏原第 10 步，自动/手动注册在第 9 步保存账号后直接结束。完整 2FA、Passkey 和免 2FA 都以 `unknown` 进入 Free，不再在注册轮次中请求 GCash 资格接口。
- GCash 资格接口与旧第 10 步执行代码继续保留，供 Free 账号面板的单个/批量手动复检使用：请求体包含 `check_gcash_pm: true`，并使用独立服务 Bearer 令牌。响应优先读取 `gcash_pm_eligible` / `gcash_pm_eligible_reason`，同时保留旧 `token_ok` / `eligible` 响应兼容。
- 侧栏首屏新增“在线工具”入口栏，公开展示 `GCash 资格查询`与`CDK 兑换中心`；点击后分别在新标签页打开 `https://gcash.20000408.xyz/` 和 `https://cdk.334401.xyz/`。入口不自动传递邮箱、密码、2FA、AT、Session 或 Cookie，插件内置 CDK 兑换能力仍保持移除。
- 账号面板只保留 `Free 组` 和 `无资格 Free 组`。`eligible`、`unknown`、`checking`、`failed` 留在 Free；只有明确 `ineligible` 进入无资格 Free。
- 第 9 步先把账号凭据及可用的 `/api/auth/session` 完整响应写入账号生命周期和 V3 结果，再完成本轮。存储失败保留当前登录现场，不误报完成。
- 顶部汇总显示总账号、有资格、待检测、检测失败、无资格和缺 AT；列表保留启用、邮箱、资格状态、登录、复检和删除。
- 保留文本/V3 JSON 导入、两个分组独立导出和删除、补充/刷新 AT、单个/批量资格复检。分组导出新增 `AT` / `Session` 选择：AT 模式保持原字段，Session 模式输出注册最终步骤或补充任务保存的完整 Session JSON并移除独立 AT 字段；旧账号缺少 Session 时会在导出结果和界面中明确提示。自动注册运行中只允许查看和导出。
- 两个分组新增“缺 Session”统计和持久化“补充 Session”任务。任务只处理启用且缺完整 Session 的账号，逐个登录并实时同步 Session 与同次 AT，支持停止、Service Worker 中断后继续和部分失败重试；完成后由 Background 自动下载本任务成功账号的 Session TXT。
- 补充 Session 任务在开始处理账号时立即显示当前序号，并在任务卡持续显示成功、失败和跳过数量；任务列表在账号面板打开时自动刷新续跑后的最新状态。“事件”按钮会把事件区带入视口，失败事件显示脱敏后的错误码和具体原因，通用任务事件使用中文文案。
- Session 任务 payload/checkpoint/result 只保存邮箱、索引、计数、文件名和下载状态，不保存密码、2FA、验证码、Cookie、AT 或 Session。Session 邮箱不一致和残缺响应不会落库，账号级失败继续下一条，存储失败立即停止整批并保留此前成功记录。

### 移除能力

- 删除 UPI、IDEAL、PIX 卡密池、CDK 路由、兑换任务、远端刷新、兑换锁、兑换历史和渠道切换逻辑。
- 删除 UPI Plus、IDEAL Plus、PIX Plus、全部 Plus 操作、Plus 识别/验证、手动移动分组和 Plus 专用导出。
- 删除 `redeem`、`verify_membership` 任务和旧兑换/Plus 消息；资格复检使用独立 `check_eligibility`。
- 删除 `chong.nerver.cc` 权限和 DNR 规则；保留 `cha.nerver.cc` 以支持资格检测、2FA 和 Passkey 登录。

### 升级迁移

- 使用 `freeAccountToolV3MigrationCompleted` 执行一次性迁移。保留账号身份、密码、2FA/Passkey、AT、取件地址、启用状态、有效性和明确资格证据。
- 旧 `paid` 行、Plus 分类、渠道、`redemption` 子树、三套卡密池、兼容别名、usage、兑换配置和旧任务会被清除。
- 先写入 V3 结果和清理后的账号记录，再删除旧键；写入失败保留旧数据，下次启动重试。
- 历史 Plus 账号只保留基础账号记录，必须经过资格复检产生新的 Free 结果后才会重新显示。

### 工作流重构

- 注册工作流升级为 `workflowVersion: 2`，完整 2FA、Passkey 和免 2FA 三条路线统一显示 10 个正式位置；侧栏删除合成的“步骤 3.5”展示行，已有账号 TOTP 登录成为可独立恢复的正式第 4 步，进度统一按 `10 / 10` 计算。
- 第 3 步现在只判断密码提交后的页面分支：新账号进入邮箱验证时原子跳过第 4 步，TOTP 挑战保留第 4 步待执行。第 5 步若防御性再次发现 TOTP 页面，会委托正式第 4 步，不维护第二套动态码登录逻辑。
- 原设置 GPT 密码节点拆为第 7 步“收取并提交密码验证码”和第 8 步“设置并持久化密码”。两步仅通过同邮箱的新密码页检查点交接；第 8 步不会重新轮询邮箱，会话失效时自动运行回退第 7 步，手动运行明确停止。
- 原安全因子与资格合并节点拆为第 9、10 步。第 9 步只完成 TOTP/Passkey 或路线跳过，不再调用资格服务；第 10 步统一复用 Session/AT 邮箱一致性校验和现有资格服务。已有 TOTP 账号只校验归档，不重复开通。
- 免 2FA 路线固定将第 7–9 步初始化为 `skipped`，第 10 步继续使用 `persist-no-2fa-free`；已有 TOTP 账号即使走该保存节点也会保留真实密码和 TOTP 凭据。
- 启动、导入和自动续跑会把旧七步状态幂等迁移到版本 2：遗留 `running` 恢复为 `pending`，旧密码节点未完成时从第 7 步重启，旧安全因子完成证据按资格状态拆分到第 9、10 步。运行中仍禁止切换路线。
- 自动运行的网络重试和步骤特殊策略改为按 `nodeId`/`executeKey` 匹配；第 7→8、9→10 检查点已就绪时不增加固定等待，不重复取码或调用安全因子接口。

### 修复

- 修复日文邮箱验证码页已明确显示“`不正確なコード`”，但第 4 步仍把它当成无明确错误并重复提交旧验证码，且未点击“`メールを再送信する`”的问题。验证码错误检测现在覆盖日文不正确/无效代码变体；重新发送入口支持带日文句尾的按钮文案，并从 `data-dd-action-name`、`data-testid` 等结构化属性逐字段后备识别。页面确认拒绝验证码后会排除旧码并请求新验证码，不再进入同码慢跳转重提。
- 修复指纹浏览器法语 ChatGPT 首页已显示“Inscription gratuite”，但步骤 2 首次执行仍等待约 25 秒后误报注册入口不可用、必须手动重试第二次的问题。共享检测器现在识别严格匹配的法语注册/登录按钮；主页探测和邮箱提交均标记为由 Background 统一裁决，内容脚本的可恢复错误只作为返回值交给当前轮打开 `/auth/login` 继续处理，不再抢先把节点广播为失败。恢复耗尽后的真实错误仍正常停止。
- 修复邮箱验证码页明确显示官网“使用密码继续”，但按钮实际指向 `/log-in/password` 时，第 3 步按旧保护规则主动不点击、随后却误报完成的问题。第三步现在会受控点击验证码页上这个官方入口，等待 `/create-account/password` 或 `/log-in/password` 密码页后继续填写和提交；只有从该验证码页入口进入且提交后的页面转换得到确认，才记录本轮密码有效并跳过后续重复设置。直接打开的已有账号登录密码页仍不会被当作注册创建入口。
- 修复用户停止后立即重新开始自动运行时，上一会话尚未返回的第 10 步资格请求仍能在新会话第 5 步期间回写节点状态，并错误触发第 7 步恢复的问题。自动运行、节点执行和免 2FA 第 10 步现在携带并复核会话 ID；被新一轮替换的循环、节点消息和异步结果会静默退出，不能停止或清理新会话。最近失败诊断同时忽略同一节点后来已完成的恢复性错误，避免把步骤 2 已自行恢复的入口错误导出为最终失败。
- 修复邮箱验证码页的“继续使用密码”实际链接到 `/log-in/password` 时，第 3 步把已成功定位的已有账号登录入口误报为“未能定位可点击元素”并停止整轮的问题。检测器现在单独分类该登录入口，但仍不把它当作注册密码创建、不点击它或提交新生成的密码；第 3 步改为继续官网验证码流程，保持 `signupPasswordCreated=false`，并保留步骤 7、8 后续正式设置密码。
- 修复指纹浏览器中的邮箱验证码页已经识别到法语密码入口文案，但入口由非常规标签、`display: contents` 或零尺寸点击容器承载时仍无法解析点击目标的问题。检测器现在有限扫描所有元素类型，接受由可见后代实际呈现的操作，从匹配节点解析可点击祖先或唯一可点击后代；无法定位时输出仅含标签、角色、可见/启用状态和安全链接分类的结构摘要，继续以 `SIGNUP_PASSWORD_SUBMIT_UNCERTAIN` 保留当前认证会话。
- 修复第 3 步已等待 5 秒、页面仍显示法语“Continuer avec un mot de passe”，但检测器因隐藏 Unicode、外层附加文字、开放 Shadow DOM 或组件重绘后的旧节点而继续误判入口不存在的问题。密码入口现在先规范化文本并在短候选中识别完整短语，有限遍历开放 Shadow DOM 和同源子框架，从文字容器解析可点击父级或子级；延迟点击前会重新定位当前控件。页面仍含密码入口迹象却无法解析时会保留会话并停止，不再把第三步记为完成。
- 修复邮箱验证码页的“继续使用密码”文字位于嵌套 `span/div`、父级组件又不带标准按钮标签时，第 3 步扫描 5 秒后仍误判入口不存在的问题。检测器现在会从精确文字叶节点向上解析点击组件；页面文本明确包含密码入口但仍无法定位操作元素时会保留会话并停止，不再继续验证码流程。
- 修复法语“继续使用密码”按钮的可见文字、`aria-label` 或遥测属性同时存在时，检测器把多个字段拼接后导致整句匹配失败，第 3 步瞬间完成并继续验证码流程的问题。密码入口现在逐字段识别，并等待最多 5 秒完成渲染和启用；入口可见但不可点击时保留认证会话并停止，不再静默降级。
- 修复法语邮箱验证码页的“Continuer avec un mot de passe”按钮没有注册链接时，第 3 步无法识别和点击的问题；法语“继续使用密码/使用密码”同义文本现在也会进入既有注册密码页切换流程，登录密码页排除规则保持不变。
- 修复 OpenAI 默认把邮箱注册导向验证码页后，第 2 步直接跳过第 3 步，导致扩展无法使用官网“使用密码继续”创建密码的问题。第 3 步现在会识别官网注册链接，跨页面恢复后复用原生密码表单提交；后台确认密码已创建时自动跳过第 7、8 步。入口不存在时保留免密码流程，`/log-in/password` 继续作为已有账号验证分支；未增加接口直调、权限或敏感请求日志。
- 修复 Free 账号落库时把整份运行状态连同上一版 `freeAccountResults` 递归写入每个账号行，少量账号即可膨胀到数十 MiB，最终使 `GET_STATE` 和保存响应超过 Chrome 单条消息 `64 MiB` 上限，表现为设置、邮箱池和账号列表同时恢复为空的问题。账号行改用持久字段白名单；已完成 V3 迁移的旧数据也会执行一次性压缩修复并保留账号凭据、AT、完整 Session、Passkey 和资格证据。启动与设置响应只发送轻量状态摘要，完整账号数据由账号面板专用接口按需读取。
- 修复设置实际已写入 Local 和 Background，但重载侧栏后部分输入框仍显示 HTML 默认值，用户误以为配置没有保存的问题；同时补上此前没有点击事件的“保存”按钮和未挂载的设置区通用自动保存入口。下拉框/复选框在变更时立即保存，连续输入保留防抖，并通过串行队列避免并发旧响应覆盖新值。首次恢复现在会回填自动重试、延迟、线程间隔、Cookie 清理和授权超时等字段。
- 修复账号和日志较多时，扩展在资源受限的指纹浏览器中自动运行明显卡顿的问题。每条日志过去都会加载完整状态（包括 Local 中的账号结果和运行历史），随后再次读取整个 Session，并把完整 `runtimeState` 随日志数组重复写回；现在日志只读取 Session 日志缓冲区，普通日志/设置 patch 只写实际变化字段，节点和流程状态更新仍保留完整运行态合并。
- 修复同一指纹浏览器 Profile 重启后，旧 `chrome.storage.session` 快照覆盖 `storage.local` 已保存配置，导致免 2FA 路线、自定义资格/TOTP API、取码等待时间和邮箱服务恢复为默认值的问题。持久配置现在只以 Local 为权威来源，后台状态更新会同步持久化配置键，Session 不再保存配置副本；旧版 Session 中的邮箱池状态仍进行兼容合并。
- 修复免 2FA 手动流程中，用户已处理前六步且第 7–9 步按路线跳过后，第 10 步仍因待处理的条件性第 4 步而灰置，只能回头操作第 4/7 步的问题。未触发 TOTP 挑战时，Side Panel 与 Background 现在都把第 4 步视为非阻塞条件节点；点击第 10 步后先将第 4 步标为跳过，再执行资格检测并正常完成 `10 / 10`。真实 TOTP 挑战仍必须先完成第 4 步，第 10 步仍不能人工跳过。
- 修复免 2FA 第 10 步保存完整 Session/Free 结果时触发 `Resource::kQuotaBytes quota exceeded`，随后被通用认证后恢复器错误拉回第 7 步的问题。Manifest 新增 `unlimitedStorage`；配额错误统一为 `FREE_ACCOUNT_STORAGE_QUOTA_EXCEEDED`，自动恢复只有限重试当前第 10 步，耗尽后保留账号和登录现场停止。
- 修复免 2FA 第 10 步资格接口请求超时或返回可重试 `failed` 后，通用认证恢复器仍探测页面并重置第 7–10 步的问题。`UPI_ELIGIBILITY_CHECK_FAILED`、`FREE_ACCOUNT_ELIGIBILITY_TIMEOUT` 和带 `trialEligibilityStatus: failed` 的临时错误现在只有限重试当前第 10 步；重试耗尽后保留账号和登录现场，错误码不再被改写为存储配额错误。
- 修复第 5 步提交资料后收到 `user_already_exists` 时，最近失败诊断错误显示为“步骤 4”，并把 `SIGNUP_USER_ALREADY_EXISTS` 结构化错误码误遮盖为令牌的问题。第 5 步现在明确说明邮箱已注册、当前轮结束并排除该邮箱；诊断保留大写下划线错误码，同时继续遮盖 JWT、AT、密码、验证码和长令牌。
- 修复长时间补充 Session 时，事件面板每次自动刷新都强制滚到底部，用户无法向上查看早期失败；同时事件仓库按最近顺序压缩，早期错误可能被后续成功和进度事件挤掉的问题。用户向上滚动后现在保留当前位置，只有原本接近底部时才继续跟随最新事件；有界事件压缩优先保留错误事件，并累计显示已压缩数量。
- 修复 Session 读取器已经返回完整 `/api/auth/session`，但批量任务因响应带有 `{ ok, status, session, accessToken, email }` 外壳而统一报 `FREE_ACCOUNT_SESSION_INCOMPLETE` 的问题。账号服务现在解包并保存内层完整 Session，同时兼容直接返回完整 Session 的登录器；空响应、缺用户、缺 AT 和邮箱不一致仍拒绝落库。
- 修复补充 Session 运行中任务卡已更新到新序号，但打开的事件面板仍永远停留在首次读取内容的问题。任务面板轮询现在会记住当前打开的任务并同步刷新其事件，最新失败原因自动滚动可见；关闭事件后停止额外事件请求。
- 修复自动注册曾被停止后再启动“补充 Session”，任务虽然连续处理账号却始终成功 `0` 的问题。`fill_session` 在开始和断点继续执行前会清除遗留的全局停止标志；运行途中收到新的全局停止信号时会整批安全停止，不再把后续账号逐条误记为登录失败。
- 修复 V3 AT/Session TXT 导出最后一列直接显示 Unix 秒数、用户无法直观看懂记录时间的问题。秒级、毫秒级和 ISO 时间来源现在统一导出为北京时间 `YYYY-MM-DD HH:mm:ss +08:00`；只改变文本显示，不修改账号存储时间或完整 Session 内容。
- 修复 Free 与无资格 Free 的 AT/Session 分组导出被错误实现为 V3 `.json` 文件的问题。下载现在统一为 UTF-8 `.txt`，每个账号一行并使用 `---` 分隔；AT 模式输出独立 AT，Session 模式把第 10 步保存的完整 `/api/auth/session` 响应作为该行凭据字段，不再附带 `schemaVersion`、`items`、资格统计等 JSON 外壳。
- 修复从免 2FA 路线切换回完整 2FA 时，第 7、8 步仍显示“跳过”、顶部仍显示 `security-factor-not-required 已完成`，且路线配置无法保存的问题。Side Panel 现在会在路线变化时立即清空旧路线节点并应用新路线默认状态；Background 设置路由补齐工厂作用域，确保新路线、空 `currentNodeId` 和新节点图能够持久化。
- 修复 Free 组与无资格 Free 组工具栏的公共操作起点和顺序不一致、按钮换行后明显不对称的问题。两个分组现在都按“凭据、导出、删除全部、补充 AT、刷新 AT、批量复检资格”的固定顺序显示公共操作；“导入 Free”和“取件地址”移入单独的 Free 专属操作行。
- 修复深色主题下 AT/Session 导出选择框展开后使用白色系统选项背景、`Session` 仍沿用浅色文字而几乎不可见的问题。选择框现在显式声明明暗 `color-scheme`，并分别设置浅色主题白底深字、深色主题深底浅字的原生选项配色。
- 修复第 10 步“验证资格并进入 Free”被通用人工跳过集合错误包含、侧栏仍显示单独“跳过”按钮的问题。完整 2FA、Passkey 和免 2FA 路线的最终资格节点现在都不创建跳过按钮；Side Panel 与 Background 同时拒绝跳过请求，确保资格结论和 Free 分组结果必须先落库。第 7–9 步原有可跳过行为不变。
- 修复 V3 的两个 Free 分组虽然已经渲染，却只能通过页面最底部日志栏中的“记录”按钮打开，用户在常用首屏完全找不到账号入口的问题。“Free 账号”入口现已移到顶栏，点击后显示 Free 与无资格 Free 两个分组；弹窗宽度同步扩展，并验证窄侧栏下按钮换行和内容不溢出。
- 修复免 2FA 路线第 9 步虽然属于固定路线跳过，却被通用 `skipped` 样式显示成蓝色“跳过”高亮的问题。所有 `route-skipped` 节点现在统一使用灰置的“当前路线跳过”展示；底层仍保留 `skipped` 状态参与进度计算。
- 修复免 2FA 自动运行完成一轮后，侧栏第 10 步仍沿用上一轮“完成”高亮的问题。轮次重置消息现在携带后台按当前路线生成的权威节点状态，侧栏不再使用初始化阶段缓存的其它路线节点键；新一轮开始时第 10 步恢复待执行，进度计数与真实后台状态一致。
- 修复选择“免 2FA Free”路线后，自动运行在新一轮初始化时丢失 `registrationFreeRoute`，按完整 2FA 默认节点图执行第 7 步的问题。自动运行现在会在重置后恢复路线设置，并按恢复后的路线重新生成节点状态；免 2FA 的第 7–9 步保持 `skipped`，只执行第 10 步资格保存。
- 修复第 9 步安全因子已完成、第 10 步明确返回 `UPI_ACCOUNT_INELIGIBLE` 后，错误在到达轮次失败策略前被通用认证恢复逻辑抢先处理，导致同一无资格账号又回到第 7 步重新设置密码的问题。结构化无资格结果现在直接交给外层策略：保留无资格生命周期和邮箱池排除状态，结束当前轮并切换下一个账号；临时资格接口失败仍沿用原有限重试。
- 修复第 8 步提交 GPT 新密码后，韩文“비밀번호를 재사용할 수 없습니다”（不能重复使用密码）未被识别，流程仍停留在 `new_password_page` 并每隔约 60 秒重复提交同一密码，最终反复回退第 7 步的问题。现在 Content Script 会提取并分类该字段错误，Background 复用既有密码替换路径生成、持久化不同密码后重试；不改变邮箱取码、账号身份、Cookie 或外层恢复边界。
- 修复第 7 步停在 ChatGPT `#settings/Security` 时，韩文“会话已过期”登录弹窗被当作 Password 入口仍在渲染，随后反复等待并最终报内容脚本 75 秒未响应的问题。现在会把多语言会话过期弹窗识别为终止状态，在入口等待期间立即返回 `SET_GPT_PASSWORD_SESSION_EXPIRED` 并提示重新登录；不会自动点击登录、清 Cookie 或切换账号。
- 修复第 7 步点击 ChatGPT Password 后进入 `First, verify it’s you` 当前密码确认页时，被误判为未知状态并反复重启第 7 步，最终导致第 8 步缺少检查点的问题。现在会识别 `/log-in/password`，校验页面邮箱与本轮邮箱一致后复用该账号已绑定密码完成确认，再继续验证码或新密码页面；不会借用其它账号密码，也不会让第 8 步自行取码。
- 修复第 10 步明确判定账号无试用资格后，非自定义邮箱池来源没有同步失效、下一次自动运行可能直接再次选择同一账号的问题。无资格结论现在统一写入账号生命周期，排除并推进邮箱池条目，复用已有 Provider 不可用处理清理当前注册身份；临时网络失败仍不会被误判为无资格。
- 第 4 步现在把 OpenAI 认证页的 `error_code: account_deactivated` 作为账号永久停用信号：手动执行会保存封禁状态并停止，自动运行会排除当前账号、清理旧认证现场，并在同一目标轮次立即选择下一个可用账号；该换号不依赖普通失败跳过开关，也不消耗普通重试次数。
- 封禁账号会写入统一账号生命周期；已有密码、TOTP 和历史记录继续保留，已确认不可用的 AT 会被清除。自定义邮箱池条目显示“账号已封禁”，保留审计信息但不再参与选择。
- 封禁换号增加同轮重复账号保护；账号池耗尽时明确停止，不点击认证页 Try again，也不继续获取、轮询或提交验证码。
...
- 修复步骤 6 连续取码至上限后把“取件页返回网页而非验证码”等临时错误交给自动运行整轮重开的缺陷。现在会保留当前 ChatGPT/认证标签页、邮箱和 Cookie 后停止，供用户从当前步骤 6 继续；不会清理现场、换邮箱或回到步骤 1。诊断只保留安全摘要，不再写入整段 HTML 取件页。
- 修复步骤 6 使用普通自定义取件 URL 时，即使浏览器请求声明 `no-store` 仍可能被上游缓存的旧邮件页面。此类直连请求现在附带一次性缓存键；截图所示“Log in to ChatGPT + temporary code”邮件仍按验证码解析，新邮件可在 Resend 后被重新拉取。已被 OpenAI 拒绝的旧码仍不会重复提交。
...
- 加强步骤 3.5 身份绑定：认证路由不再误判为已登录，提交前必须匹配页面邮箱，成功只接受当前步骤 3.5 标签页上与本轮邮箱一致的 `/api/auth/session`，不再借用其他 ChatGPT 标签页 Session。
- 步骤 3.5 失败现在跨消息链保留 `code`、`retryable` 和 `preserveSignupSession`；自动运行识别专用终止动作后立即停机，不重试、不清 Cookie、不换邮箱、不重新注册。
- 已有 TOTP 账号即使继续执行 Free 资格保存节点，也会保留原密码/TOTP 并写入 `twoFactorEnabled=true`、`no2faFreeRoute=false`；只有真正免 2FA 路线才写 `no2faFreeRecordedAt` 和空凭据。凭据缺失时不再提前写状态或标记邮箱已用。
- 修复步骤 3.5 分格验证码日志可包含完整动态码或分格当前值的问题；内容脚本不再构造敏感文本，orchestrator 同时做集中脱敏作为第二层保护。
