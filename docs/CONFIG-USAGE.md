...
打开顶部“配置”菜单，点击“导出最近一次失败诊断”。扩展会把脱敏 JSON 直接写入系统剪贴板，并提示“已导出至剪贴板”。

诊断内容包括最后一次失败、失败前后各 100 条日志、当前页面 URL、工作流和认证页面状态，以及验证码输入框检测结果。验证码、密码、完整 AT、Cookie、CDK、URL 参数值和完整邮箱不会写入剪贴板。页面内容脚本不可用时，仍会导出已有日志、当前 URL 和安全的探测错误。
