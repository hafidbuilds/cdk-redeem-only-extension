...
function getResetPasswordSuccessText() {
  const pattern = /(?:password\s+(?:has\s+been\s+)?(?:updated|reset|changed|set)\s+successfully|successfully\s+(?:updated|reset|changed|set)\s+(?:your\s+)?password|your\s+password\s+(?:was|has\s+been)\s+(?:updated|reset|changed|set)|密码.{0,32}(?:设置|重置|更新|修改).{0,20}(?:成功|完成)|(?:成功|已完成).{0,20}(?:设置|重置|更新|修改).{0,24}密码|パスワード.{0,32}(?:設定|変更|更新|リセット).{0,20}(?:完了|成功)|पासवर्ड.{0,40}(?:सफलतापूर्वक|अपडेट|रीसेट|बदल))/i;
  const candidates = Array.from(document.querySelectorAll(
    '[role="alert"], [role="status"], [aria-live], [data-testid*="toast" i], [data-testid*="success" i], .toast, .success, p, div'
  ));
  for (const element of candidates) {
    if (!isVisibleElement(element)) continue;
    const text = String(element.textContent || '').replace(/\s+/g, ' ').trim();
    if (text && text.length <= 400 && pattern.test(text)) {
      return text;
    }
  }
  const pageText = getPageTextSnapshot();
  return pattern.test(pageText) ? pageText.slice(0, 400) : '';
}
...
async function startSetGptPasswordResetFlow(payload = {}) {
  ...
  const passwordActionWaitMs = Math.max(
    5000,
    Math.min(60000, Math.floor(Number(payload.passwordActionWaitMs) || 25000))
  );
  ...
  const passwordAction = await waitForChatGptSettingsPasswordAction(passwordActionWaitMs);
  if (!passwordAction) {
...
async function waitForSetGptPasswordSubmitOutcome(visibleStep, timeout = 30000, options = {}) {
  ...
  while (Date.now() - start < timeout) {
    ...
    if (snapshot.state === 'password_updated_page') {
      return {
        success: true,
        gptPasswordSet: true,
        confirmedBySuccessMessage: true,
        successText: snapshot.successText || '',
        url: snapshot.url || lastUrl,
  ...
  if (finalSnapshot.state === 'password_updated_page') {
    return {
      success: true,
      gptPasswordSet: true,
      confirmedBySuccessMessage: true,
      successText: finalSnapshot.successText || '',
      url: finalSnapshot.url || lastUrl,
      authRetryRecoveredCount: authRetryRecoveryCount,
      submitClickCount,
    };
  }
