...
const remoteOperationPolicy = self.MultiPageRemoteOperationPolicy.createRemoteOperationPolicy({ chromeApi: chrome });
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    try {
      ...
      if (/^\d{6}$/.test(code) && !/^0{6}$/.test(code)) {
        await addLog(`步骤 ${step}：已通过 iCloud API 找到验证码：${code}`, 'ok');
        return {
          ok: true,
          code,
      ...
      if (code) {
        lastError = new Error(`步骤 ${step}：iCloud API 返回了无效验证码，已忽略占位值。`);
        await addLog(lastError.message, 'warn');
      }

      lastError = lastError || new Error(`步骤 ${step}：iCloud API 暂未返回验证码（${attempt}/${maxAttempts}）。`);
      await addLog(lastError.message, attempt === maxAttempts ? 'warn' : 'info');
    } catch (error) {
      lastError = error;
...
function isSignupPasswordPageUrl(rawUrl) {
  ...
  return isSignupPageHost(parsed.hostname)
    && /\/(?:u\/)?(?:create-account|signup|log-in|login)\/password(?:[/?#]|$)/i.test(parsed.pathname || '');
}
